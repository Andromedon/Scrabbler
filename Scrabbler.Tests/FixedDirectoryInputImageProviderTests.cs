...
public sealed class FixedDirectoryInputImageProviderTests : IDisposable
{
    ...
    [Fact]
    public void ReturnsNullWhenDirectoryHasNoImages()
    {
        File.WriteAllText(Path.Combine(_directory, "notes.txt"), "ignore");

        var provider = new FixedDirectoryInputImageProvider(_directory);

        Assert.Null(provider.GetSelectedImage());
    }

    [Fact]
    public void SelectsNewestSupportedImage()
    {
        ...
        Assert.Equal(newImage, provider.GetSelectedImage()!.FullName);
        Assert.NotEqual(oldImage, provider.GetSelectedImage()!.FullName);
    }
